const { AdviserPlacementType, ItemType, ChildrenPlacementType, OrgItemConfig, Colors } = require('basicprimitives');
...
function GetNextPhoto() {
  ...
  return "/api/images/photos/" + photos[photoIndex] + ".png";
}
...
var names = ["David Dalton", "Jeanna White", "James Holt", "Thomas Williams", "David Kirby", "Lynette Maloney", "Glen Zeigler", "Ione Gallegos", "Jose Clark", "Heather Simmons",
  ...
  for (var groupKey in groups) {
    ...
    for (var levelIndex = 0; levelIndex < group.levels.length; levelIndex += 1) {
      ...
      for (var index = 0; index < levelSize; index += 1) {
        counter++;
        var memberItem = new OrgItemConfig({
          id: counter,
          parent: manager.id,
          title: GetNextName(),
