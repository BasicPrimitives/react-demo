...
class InteractivityOptionsPanel extends Component { // eslint-disable-line react/prefer-stateless-function
  ...
  render() {
    ...
    return (
      <div>
        <h4>Interactivity</h4>
        <p>Use this option to disable mouse highlight on touch devices.</p>
        <RadioGroupOption
        ...
        />
        <p>This option defines highlight gravity radius, so minimized item gets highlighted when mouse pointer gets into marker gravity radius boundaries.</p>
        <ComboBoxOption
        ...
        />
        <CheckboxOption caption="Enable Panning" propertyName="enablePanning" value={config.enablePanning} onChange={value => setOption('enablePanning', value)} />
      </div>
    );
  }
}
