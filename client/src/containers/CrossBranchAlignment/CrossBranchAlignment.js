...
import {
  ...
  setTemplateOption,
  UserActionType
} from 'redux/modules/demos/crossbranchalignment';
...
function CrossBranchAlignment() {
  const loaded = useSelector(state => state.crossbranchalignment.loaded); 
  const markdown = useSelector(state => state.crossbranchalignment.markdown); 
  const centerOnCursor = useSelector(state => { 
    return state.crossbranchalignment.centerOnCursor
  }); 
  const config = useSelector(state => state.crossbranchalignment.config); 
  const userAction = useSelector(state => state.crossbranchalignment.userAction); 
  const itemsHash = useSelector(state => state.crossbranchalignment.itemsHash); 
  ...
  return (
    <>
      <Helmet>
        <title>Cross-Branch Children Alignment Demo</title>
        <meta name="description" content="Cross-branch children alignment demo in organizational chart. Open sources. Universal React architecture. Supports client side PDf rendering and download." />
      </Helmet>
      <h1>Cross-Branch Children Alignment</h1>
      ...
      {isPdfDialogOpen && <OrgPdfViewDialog 
        ...
        config={config}
        fileName="crossbranchalignment.pdf"
        caption="Cross-Branch Children Alignment Demo"
...
export default CrossBranchAlignment;
